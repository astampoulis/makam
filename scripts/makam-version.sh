...
check-if-updated)
  ...
  PARENTVERSION=$(git show master:toploop/version.ml | get_version)

  if [[ ! -z $(git diff-tree -r --name-status $PARENT..HEAD termlang toploop stdlib opam npm/src npm/package.json) && $PARENTVERSION == $BASEVERSION ]]; then
    cat <<EOF
Summary of changes:

$(git diff-tree -r --name-status $PARENT..HEAD termlang toploop stdlib opam npm/src npm/package.json)
